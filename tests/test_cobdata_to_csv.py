...
class TestCobdataToCsv(unittest.TestCase):
    def test_cobol_data_to_dict_pic_x(self):
        ...
        """
        rec_len, layout = cobdata_to_csv.copybook_to_layout(copybook)
        rec_data = b'123456'
        rec_dict = cobdata_to_csv.cobol_data_to_dict(rec_data, layout, input_encoding='utf8')
        print(rec_dict)
    ...
    def test_cobol_data_to_dict_pic_9_display(self):
        ...
        rec_dict = cobdata_to_csv.cobol_data_to_dict(rec_data, layout)
        print(rec_dict)
        self.assertEqual(rec_dict['MY_FIELD_1'], '123456')  # unsigned
        self.assertEqual(rec_dict['MY_FIELD_2'], '-123456')  # signed
...
class TestFunctions(unittest.TestCase):
    def test_add_decimal_point(self):
        self.assertEqual(1, cobdata_to_csv.add_decimal_point(1, 0))
        self.assertEqual(0.1, cobdata_to_csv.add_decimal_point(1, 1))
        self.assertEqual('1', cobdata_to_csv.add_decimal_point('1', 0))
        self.assertEqual('0.1', cobdata_to_csv.add_decimal_point('01', 1))

    def test_unpack_zd(self):
        self.assertEqual('', cobdata_to_csv.unpack_zd('', 0), 'No value passed')

    def test_uppack_comp(self):
        self.assertEqual('+0', cobdata_to_csv.unpack_comp('', 2, 0), 'No value passed')

    def test_uppack_comp3(self):
        self.assertEqual('', cobdata_to_csv.unpack_comp3('', 0), 'No value passed')
